<script lang="ts">
	...
	import { buildSmoothPath, formatCount, formatTooltipDate, hexAlpha } from './chart-utils';
	...
	const WIDTH = 800;
	const HEIGHT = 220;
	const PADDING = { l: 50, r: 12, t: 12, b: 24 };
	...
	const linePath = $derived(buildSmoothPath(projection.points));
			...
			<g>
				...
				<g pointer-events="none">
					...
					<text
						...
						font-weight="600"
					>
						{formatCount(pt.raw.value)} followers
					</text>
					<text
						x={tooltipBox.tx + 10}
						y={tooltipBox.ty + 28}
